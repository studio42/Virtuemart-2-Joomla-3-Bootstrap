...
function virtuemartBuildRoute(&$query) {
	...
	// admin simplified links
	if ( isset($query['tmpl']) && $query['tmpl'] ==='component' ){
		$query['Itemid'] = $jmenu['admin'];
		if (isset($query['view']) ) $segments[] = $helper->lang($query['view']);
		unset($query['view']);
	}
	...
	switch ($view) {
		...
		case 'category';
			...
			} elseif ( isset($jmenu['category']) ) $query['Itemid'] = $jmenu['category'];
		...
		case 'manufacturer';
			...
			if(isset($query['virtuemart_manufacturer_id'])) {
				...
				if ( isset($jmenu['manufacturer']) ) $query['Itemid'] = $jmenu['manufacturer'];
					...
					$query['Itemid'] = $jmenu['virtuemart'];
					$segments[] = $helper->lang('manufacturers');
				}
			}
			break;
		case 'user';
...
function virtuemartParseRoute($segments) {
	...
	// add for admin in front
	$jinput = JFactory::getApplication()->input;
	if ( $jinput->get('tmpl', '', 'word') == 'component' || $helper->activeMenu->view =='admin') $admin = true;
	else $admin = false;
	...
	if (  $helper->compareKey($segments[0] ,'manufacturer') ) {
		array_shift($segments);
		// temp fix
		if (empty($segments) && $admin) { 
			$vars['view'] = 'manufacturer'; // admin front add
			return $vars; 
		}
...
// if joomla link joomlalink/vendorname/layout
	...
	// NOTE, this was removed by the team
	...
	if ( $helper->compareKey($view,'orders') || $helper->activeMenu->view == 'orders') {
		...
		if ( !empty($segments) ) {
			...
			$vars['layout'] = $admin ? 'order' : 'details';
		}
		return $vars;
	}
...
class vmrouterHelper {
	...
	/* language array */
	public $lang = null ;
	public $Jlang = null ; // joomla language keys in instance
	public $langTag = null ;
	public $query = array();
	...
	private $dbview = array('vendor' =>'vendor','category' =>'category','productdetails' =>'product','cart' => 'cart','manufacturer' => 'manufacturer','user'=>'user');
	...
	private function setLangs($instanceKey){
	...
	private function getCategoryRouteNocache($virtuemart_category_id){
	...
	private function getCategoryRecurse($virtuemart_category_id,$catMenuId,$first=true ) {
		static $idsArr = array();
		...
		if (isset ($ids->child)) {
			$idsArr[] = $ids->child;
			if (in_array($ids->parent,$idsArr)) {
				return $idsArr;// STUDIO42 Fix, infinite loop if child is parent !!!
			}
	...
	private function setMenuItemIdJ17(){
		...
		if(empty($menuItems)){
			...
			foreach ($menuItems as $item)	{
				...
				// admin menu links
				if($item->link ==='index.php?option=com_virtuemart&view=admin') $link['view'] = 'admin';
				...
				if(!empty($link['view'])){
					...
					if ( $dbKey && isset($link['virtuemart_'.$dbKey.'_id'])){
		...
		if ( !isset( $this->menu['category']) ) {
			$this->menu['category'] = $this->menu['virtuemart'];
		}
	...
	public function lang($key) {
		if ($this->seo_translate ) {
			...
			if ($this->Jlang->hasKey('COM_VIRTUEMART_SEF_'.$jtext) ){
				...
				return $this->Jlang->_('COM_VIRTUEMART_SEF_'.$jtext);
			}
		}
